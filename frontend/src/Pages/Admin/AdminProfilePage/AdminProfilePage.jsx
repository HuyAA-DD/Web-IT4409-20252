...
export default function AdminProfilePage() {
  ...
  const { handleAvatarUpdate, handleAdminNameUpdate } = useOutletContext?.() || {};
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        console.error('Lỗi khi tải thông tin admin profile', error);
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      setUser(updatedUser);
      handleAdminNameUpdate?.(updatedUser.fullName);
      setFormValues({ fullName: updatedUser.fullName || '', email: updatedUser.email || '' });
      setIsEditing(false);
      message.success('Cập nhật thông tin thành công!');

      const currentLocalData = getStoredAuth();
      if (currentLocalData) saveAuthUser({ ...currentLocalData, fullName: updatedUser.fullName, email: updatedUser.email });
    } catch (error) {
  ...
  const handleUploadAvatar = async ({ file, onSuccess, onError }) => {
    try {
      ...
      const currentLocalData = getStoredAuth();
      if (currentLocalData) saveAuthUser({ ...currentLocalData, avatarUrl: newAvatarUrl });
    ...
    <div className="w-full max-w-7xl mx-auto pb-8">
      <div className="relative bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm mb-6">
        <div className="h-40 sm:h-48 md:h-56 bg-orange-600 relative">
          <img alt="Profile Cover" className="w-full h-full object-cover mix-blend-overlay opacity-40" src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80" />
        </div>
        ...
        <div className="px-6 sm:px-8 pb-8 flex flex-col md:flex-row items-center md:items-end gap-6 -mt-16 md:-mt-20 relative z-10 text-center md:text-left">
          <div className="relative group shrink-0">
            <img alt="Admin Avatar" className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white shadow-lg bg-white object-cover" src={user.avatarUrl || 'https://via.placeholder.com/150'} />
            <div className="absolute inset-0 flex items-center justify-center md:items-end md:pb-3 opacity-0 group-hover:opacity-100 transition-opacity bg-black/30 rounded-full md:bg-transparent">
              <Upload name="file" showUploadList={false} beforeUpload={beforeUpload} customRequest={handleUploadAvatar}>
                <button type="button" disabled={uploading} className="bg-orange-600/90 text-white p-2.5 rounded-full shadow-lg hover:bg-orange-700 transition-colors disabled:cursor-not-allowed">
          ...
          <div className="flex-1 w-full pb-2">
            <div className="flex flex-col md:flex-row items-center gap-3 justify-center md:justify-start">
              ...
              <span className="bg-orange-50 text-orange-700 border border-orange-100 px-3 py-1.5 rounded-full text-xs font-bold tracking-wider whitespace-nowrap">{user.role}</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 lg:gap-x-16 gap-y-8">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <div className="flex items-center justify-between bg-gray-50 px-4 py-3 rounded-lg border border-gray-200 overflow-hidden">
              <code className="text-sm font-mono text-orange-600 truncate mr-4">{user.id}</code>
            </div>
          </div>
