...
const AuthPage = () => {
  ...
  const getRoleRedirect = (role = 'USER') => {
    ...
    if (normalized.includes('ADMIN')) return ADMIN_ROUTE.Dashboard;
    if (normalized.includes('SELLER')) return SELLER_ROUTE.Home;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate(getRoleRedirect(authData.role));
    } catch (error) {
      console.error('Auth error:', error);
      const serverMessage = error?.response?.data?.message || error?.message || 'Lỗi xác thực. Vui lòng thử lại.';
          ...
          <div className="relative w-full md:w-1/2 bg-white flex items-center justify-center p-6 sm:p-12 overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-[480px] bg-white/95 backdrop-blur-xl p-[8px] sm:p-12 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-orange-50">
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="flex flex-col">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    {isLogin && (
                      <a className="text-sm text-orange-600 hover:text-orange-500 hover:underline font-medium" href="#">
                        Quên mật khẩu?
                      </a>
                    )}
                  </div>
                ...
                {isLogin && (
                  <div className="flex items-center gap-2 pt-2">
                    <input
                      ...
                      type="checkbox"
                    />
                    <label className="text-sm text-gray-600 cursor-pointer select-none" htmlFor="remember">
                      Ghi nhớ đăng nhập
                    </label>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <button className="h-12 flex items-center justify-center gap-2 bg-white border border-gray-200 rounded-xl font-medium text-gray-700 hover:bg-orange-50 hover:border-orange-200 transition-all active:scale-95 shadow-sm" type="button">
                  <img
                    alt="Google"
                    className="w-5 h-5"
